...
part1 = BuildWQ_Partition(m,n,k);
part2 = BuildWQ_Partition(n,m,k);
...
function WQ = BuildWQ_Partition(m,n,k)

% Build the WQ matrix for first partition
mat_W1 = zeros(n-k,n-k);

for i1 = 0:1:n-k-1
    for i2 = 0:1:n-k-1
        if i1+i2 <= n-k-1
            mat_W1(i1+1,i2+1) = (n-k-i1-i2) ./ (n-k);
        end
    end
end

vec_W1 = GetAsVector(mat_W1);
nNonZeros_W1 = nchoosek((n-k-1)+2,2);
vec_W1 = vec_W1(1:nNonZeros_W1);
W1 = diag(vec_W1);

nRows_zeroMat = (2/(n-k)) * nchoosek(n-k+1,2);
nCols_zeroMat = nchoosek(n-k+1,2);
