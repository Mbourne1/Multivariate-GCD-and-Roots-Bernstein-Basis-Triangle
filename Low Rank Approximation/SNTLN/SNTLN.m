...
DTQ_alpha = BuildDTQ(fww_wrt_alpha,alpha_gww_wrt_alpha,k);

% Calculate the derivative of T(f,g) with respect to theta_{1}
DTQ_wrt_th1 = BuildDTQ(fww_wrt_th1, alpha(ite).*gww_wrt_th1,k);

% Calcualte the derivative of T(f,g) with respect to theta_2
DTQ_wrt_th2 = BuildDTQ(fww_wrt_th2, alpha(ite).*gww_wrt_th2,k);
...
DNQ_wrt_alpha   = zeros(nRows_Sk_fg, nCols_Tf + nCols_Tg);

% Initilaise the derivative of N wrt theta_1.
DNQ_wrt_th1   = zeros(nRows_Sk_fg, nCols_Tf + nCols_Tg);

% Initialise the derivative of N wrt theta 2
DNQ_wrt_th2   = zeros(nRows_Sk_fg, nCols_Tf + nCols_Tg);
...
while condition(ite) > SETTINGS.STLN_MAX_ERROR && ite < SETTINGS.STLN_MAX_ITERATIONS
    ...
    delta_zk = y(1 : nCoeffs_fxy + nCoeffs_gxy);
    
    % Remove the zk coefficients from the vector y.
    y(1:nCoeffs_fxy + nCoeffs_gxy) = [];
    
    % Get the entries of y corresponding to x
    delta_xk = y(1:nCoeffs_x,1);
    ...
    DTQ_fg = BuildDTQ(fww, alpha(ite).*gww,k);
    ...
    DTQ_wrt_alpha = BuildDTQ(fww_wrt_alpha, alpha_gww_wrt_alpha,k);
    ...
    DNQ = BuildDTQ(z_fww,alpha(ite).*z_gww,k);
    
    % Build the coefficient matrix N with respect to alpha
    DNQ_wrt_alpha = BuildDTQ(zfw_wrt_alpha, alpha_zgw_wrt_alpha,k);
    
    % Calculate the derivatives of DNQ with respect to theta
    DNQ_wrt_th1 = BuildDTQ(zfw_wrt_th1, alpha(ite).*zgw_wrt_th1,k);
    
    % Calculate the derivatives of DNQ with respect to theta
    DNQ_wrt_th2 = BuildDTQ(zfw_wrt_th2, alpha(ite).*zgw_wrt_th2,k);
    ...
    DTNQ = BuildDTQ(fww + z_fww, alpha(ite).*(gww + z_gww), k);
    
    % Calculate the paritial derivative of D*(T+N)*Q with respect to
    % \alpha
    DTNQ_alpha = BuildDTQ(fww_wrt_alpha + zfw_wrt_alpha,...
