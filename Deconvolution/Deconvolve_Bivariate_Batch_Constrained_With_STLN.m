function [arr_hx] = Deconvolve_Bivariate_Batch_Constrained_With_STLN(arr_fxy,vDegt_fxy)
%
%
% % Inputs
%
...
% Get the number of polynomials in arr_fxy
...
while condition(ite) > SETTINGS.MAX_ERROR_DECONVOLUTIONS) && 
    ...
    delta_pxy = y(1:nCoefficients_pxy);
    delta_zxy = y(nCoefficients_pxy + 1 : end);
    
    % Add structured pertubrations to z(x,y)
...
for i = 1:1:length(unique_vMult)
    ...
    arr_px{i} = GetAsMatrix(vec_px,deg_px,deg_px);
