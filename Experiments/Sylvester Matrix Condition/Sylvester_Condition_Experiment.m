function [] = Sylvester_Condition_Experiment(m,n)
...
% %
% 
vDTQ_condition = zeros(min(m,n),1);
vDT_condition = zeros(min(m,n),1);
vT_condition = zeros(min(m,n),1);
vDTQ_exc_denom_condition = zeros(min(m,n),1);
...
for k = 1:1:min(m,n)
    ...
    % Build the matrix T
    T1 = BuildT(mat_fxy,m,n-k);
    T2 = BuildT(mat_gxy,n,m-k);
    
    % Build the matrix Q
    Q = BuildQ(m,n,k);
    
    S = D*[T1 T2]*Q;
    vDTQ_condition(k) = cond(S);

    S = D*[T1 T2];
    vDT_condition(k) = cond(S);

    S = [T1 T2];
    vT_condition(k) = cond(S);
    ...
    T2 = T2 ./ com_denom_T2;
    S = D*[T1 T2]*Q;
    vDTQ_exc_denom_condition(k) = cond(S);
end


figure('name',[mfilename ' : ' 'Conditions'])
hold on
plot(log10(vDTQ_condition),'-s','DisplayName','DTQ');
plot(log10(vDT_condition),'-s','DisplayName','DT');
plot(log10(vT_condition),'-s','DisplayName','T');
plot(log10(vDTQ_exc_denom_condition),'-s','DisplayName','DTQ - Factors Removed')
legend(gca,'show');
